...
function compare(word, guess) {
  // helper to count letters in the secret word
  const bank = {};
  for (const char of word) {
    bank[char] = (bank[char] || 0) + 1;
  }

  let result = new Array(word.length).fill('0');
  // let bank = countChars(word);
  ...
  for (let i = 0; i < word.length; i++) {
    // only check if it wasn't an exact match AND the letter exists in the bank
    if (result[i] === '0' && bank[guess[i]] > 0) {
